...
def generate_referral_text(invite_count: int):
    return (
        ...
        "1. *Нажмите Поделится* \n"
        "2. *Перешлите сообщение другу* или *поделитесь вашей ссылкой*\n\n"
        "- Друг получит *7 дней* бесплатного доступа!\n"
        "- Вы получите *3 бонусных дня* за каждого друга, подключившегося *по ссылке*\n"
        "- Если друг оформит подписку, вам начисляется *14 бонусных дней*\n\n"
...
async def handle_share(event: Message | CallbackQuery, bot: Bot):
    ...
    if isinstance(event, Message):
        ...
        await event.message.edit_text(share_message, reply_markup=share_keyboard, parse_mode="Markdown",disable_web_page_preview=True)
        await event.answer()
