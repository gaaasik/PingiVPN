...
async def create_one_time_payment(user_id, user_name, user_email, tariff_code: str):
    ...
    payment = Payment.create({
        ...
        "confirmation": {
            "type": "redirect",
            "return_url": "https://t.me/PingiVPN_bot"
        },
        "capture": True,
        ...
        "metadata": {
            "user_id": user_id,
            "user_name": user_name,
            "plan": tariff_code  # используем это позже в Redis для расчета месяцев
        },
        "receipt": {
            "customer": {
                "email": user_email
            ...
            "items": [
                {
                    "description": f"VPN: {tariff['label']}",
                    "quantity": "1.00",
                    "amount": {
                        "value": tariff["amount"],
                        "currency": "RUB"
...
async def process_payment_message(message: str, bot: Bot):
    try:
        logging.info(f"📨 Начало обработки сообщения: {message}")
        ...
        if not all([chat_id, amount, currency, status, payment_id]):
            await send_admin_log(bot, f"❌ Некорректные данные в платеже: {data}")
            return
        ...
        logging.info(f"✅ Платеж прошёл успешно. Пользователь: {chat_id}")

        if not us.servers:
            logging.error(f"🚫 Нет серверов у пользователя {chat_id}")
            return

        server = us.servers[0]

        # Извлечение тарифного плана из metadata
        plan_code = data.get("payload_json", {}).get("object", {}).get("metadata", {}).get("plan", "1")
        tariff = TARIFFS.get(plan_code, TARIFFS["1"])  # fallback
        months = tariff["months"]
        logging.info(f"📦 Применён тариф: {tariff['label']} ({months} мес.)")

        # Получаем текущую дату и дату окончания
        current_date = datetime.now()
        date_key_off_str = await server.date_key_off.get()
        ...
        if date_key_off < current_date:
            ...
            new_expiry_date = date_key_off + timedelta(days=30 * months)

        # Обновляем ключ
        await server.status_key.set("active")
        await server.date_payment_key.set(current_date.strftime("%d.%m.%Y %H:%M:%S"))
        await server.date_key_off.set(new_expiry_date.strftime("%d.%m.%Y %H:%M:%S"))
        await server.has_paid_key.set(int(await server.has_paid_key.get()) + 1)

        logging.info(f"🔐 Новый срок действия: {new_expiry_date.strftime('%d.%m.%Y %H:%M:%S')}")

        # Постоплатные действия
        await handle_post_payment_actions(bot, chat_id)

    except json.JSONDecodeError as e:
        logging.error(f"❌ JSON decode error: {e}")
    except Exception as e:
        logging.error(f"🔥 Ошибка обработки Redis-платежа: {e}")
