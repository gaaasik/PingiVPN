...
config_after_install() {
	...
	if [[ -n "${existing_webBasePath}" && ${#existing_webBasePath} -lt 4 ]]; then
        # Генерируем порт
        ...
        echo "username=${config_username}" > /tmp/x_ui_install_output.txt
        echo "password=${config_password}" >> /tmp/x_ui_install_output.txt
        echo "port=${config_port}" >> /tmp/x_ui_install_output.txt
        echo "webBasePath=${config_webBasePath}" >> /tmp/x_ui_install_output.txt
        echo "server_ip=${server_ip}" >> /tmp/x_ui_install_output.txt

        echo -e "Installation complete. Access URL: http://${server_ip}:${config_port}/${config_webBasePath}"
    fi
